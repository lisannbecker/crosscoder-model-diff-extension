...
class Trainer:
    def __init__(self, cfg, model_A, model_B, all_tokens, method = "default"):
        self.cfg = cfg
        self.model_A = model_A
        self.model_B = model_B

        if method in ["default", "finetuning"]:
            self.crosscoder = CrossCoder(cfg)
            self.buffer = Buffer(cfg, model_A, model_B, all_tokens)
        elif method in ["distillation", "scale"]:
            self.crosscoder = CrossCoderMultiscale(cfg)
            self.buffer = BufferMultiscale(cfg, model_A, model_B, all_tokens)
    ...
    def step(self):
        ...
        loss.backward()
        clip_grad_norm(self.crosscoder.parameters(), max_norm=1.0)
        self.optimizer.step()
        self.scheduler.step()
