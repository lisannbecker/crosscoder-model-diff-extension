...
def load_tokens(model_name='gemma-2-2b'):
    # load locally instead - .pt file in same directory as train.py
    here = os.path.dirname(__file__)
    if 'gemma-2' in model_name:
        pt_path = os.path.join(here, "pile-lmsys-mix-1m-tokenized-gemma-2.pt")
    elif 'pythia' in model_name:
        pt_path = os.path.join(here, "owt-1m-tokenized-pythia-160m.pt")
    elif 'gpt2' in model_name:
        pt_path = os.path.join(here, "owt-1m-tokenized-gpt2.pt")
    else:
        raise ValueError(f'Model {model_name} not supported. Use gemma-2, pythia or gpt2')
    try:
...
def load_pile_lmsys_mixed_tokens_original():
    try:
